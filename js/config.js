...
const GAME_CONFIG = {
    ...
    BASE_PARAMS: {
        "T0": {
            vpip: 0.45,      // 松凶：玩45%的牌
            pfr: 0.28,       // 高PFR：频繁加注
            three_bet: 0.15, // 适度3-bet
            bluff: 0.12,     // 适度诈唬
            error_rate: 0.08,// 低错误率（强玩家）
            playable_range: 0.55 // 宽起手牌范围（前55%）
        },
        "T1": {
            vpip: 0.30,      // 标准：玩30%的牌
            pfr: 0.22,       // 中等PFR
            three_bet: 0.10, // 中等3-bet
            bluff: 0.08,     // 中等诈唬
            error_rate: 0.15,// 中等错误率
            playable_range: 0.40 // 中等范围（前40%）
        },
        "T2": {
            vpip: 0.22,      // 紧弱：玩22%的牌
            pfr: 0.12,       // 低PFR：很少加注
            three_bet: 0.05, // 低3-bet
            bluff: 0.05,     // 低诈唬
            error_rate: 0.25,// 高错误率（弱玩家）
            playable_range: 0.30 // 紧起手牌范围（前30%）
        }
    },
    ...
    decide(player, highestBet, stage, communityCards, potSize) {
        ...
        // Case: Someone bet (toCall > 0)
            ...
            if (handStrength > callThreshold && handStrength > potOdds) {
                ...
                // T0 (3bet=0.15) → 0.705 | T2 (3bet=0.05) → 0.735
...
function calculateCrushProtection(userCurrent, userInitial) {
    ...
    if (ratio < 0.30) return 0.70; // 用户被碾压时降低难度
    if (ratio > 2.00) return 1.20; // 用户领先时增加难度
    return 1.00;
}
